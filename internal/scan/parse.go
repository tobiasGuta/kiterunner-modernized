...
func ParseDomain(domain string) ([]*http.Target, error) {
	// Attempt to parse as JSON first (httpx/subfinder support)
	if strings.HasPrefix(strings.TrimSpace(domain), "{") {
		if extracted, ok := extractTargetFromJSON(domain); ok {
			domain = extracted
		}
	}
...
func extractTargetFromJSON(input string) (string, bool) {
	var target struct {
		URL    string `json:"url"`
