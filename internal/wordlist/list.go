...
func List(ctx context.Context, opts ...ListOption) error {
	...
	default:
		...
		table.Header("alias", "filename", "source", "count", "filesize", "cached")
		for _, v := range wls {
			_ = table.Append(v.Shortname, v.Filename, v.Source, strconv.Itoa(v.LineCount), v.FileSize, strconv.FormatBool(v.Cached))
		}
		table.Render()
	}
