...
func init() {
	...
	bruteCmd.Flags().IntVar(&jitter, "jitter", 0, "jitter percentage for delay (0-100)")
	bruteCmd.Flags().BoolVar(&calibrate, "calibrate", calibrate, "enable soft 404 calibration (auto-calibration)")
	...
	bruteCmd.Flags().StringVar(&proxyListFile, "proxy-list", proxyListFile, "file containing list of proxies to rotate")
	bruteCmd.Flags().StringVar(&replayProxy, "replay-proxy", replayProxy, "upstream proxy to replay valid findings to")
