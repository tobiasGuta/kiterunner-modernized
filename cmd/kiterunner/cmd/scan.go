...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		opts := []scan.ScanOption{
			...
			scan.Calibrate(calibrate),
			scan.Jitter(jitter),
			scan.LoadProxyStream(proxyListFile),
			scan.ReplayProxy(replayProxy),
...
func init() {
	...
	scanCmd.Flags().IntVar(&jitter, "jitter", 0, "jitter percentage for delay (0-100)")
	...
	scanCmd.Flags().BoolVar(&calibrate, "calibrate", calibrate, "enable soft 404 calibration (auto-calibration)")
	...
	scanCmd.Flags().StringVar(&proxyListFile, "proxy-list", proxyListFile, "file containing list of proxies to rotate")
	scanCmd.Flags().StringVar(&replayProxy, "replay-proxy", replayProxy, "upstream proxy to replay valid findings to (e.g. http://127.0.0.1:8080)")
