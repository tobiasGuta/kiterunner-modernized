...
type Config struct {
	...
	Calibrate            bool          // Calibrate toggles the soft 404 detection mechanism
	...
	Proxies              []string
	ProxyManager         *http.ProxyManager
	ReplayProxy          string
}
...
func Calibrate(b bool) ConfigOption {
	return func(c *Config) {
		c.Calibrate = b
	}
}

func Jitter(j int) ConfigOption {
	return func(c *Config) {
		c.Jitter = j
	}
}

func Proxies(l []string) ConfigOption {
	return func(c *Config) {
		c.Proxies = l
	}
}

func ReplayProxy(url string) ConfigOption {
	return func(c *Config) {
		c.ReplayProxy = url
	}
}
