...
func NewEngine(routes http.RouteMap, opts ...ConfigOption) *Engine {
	...
	if len(e.config.Proxies) > 0 {
		e.config.ProxyManager = http.NewProxyManager(e.config.Proxies)
	}
...
func handleTarget(ctx context.Context, target *http.Target, reqChan chan *ReqMsg, rm http.RouteMap, config *Config) error {
	...
	if config.ProxyManager != nil {
		c.Dial = config.ProxyManager.GetDialer(target.IsTLS)
	}
...
func handleRequest(ctx context.Context, j *job, resChan chan *Result, config *Config) error {
	...
	// Prepare config for this request (handle proxy logic)
	reqConfig := config.HTTP
	if config.ProxyManager != nil && !j.t.IsTLS {
		reqConfig.WriteAbsoluteURI = true
	}
...
routeloop:
	for idx, route := range j.routes {
		...
		for {
			// Phase 5: Jitter and Delay
			if config.Delay > 0 {
				d := config.Delay
				if config.Jitter > 0 {
					// Calculate jitter: +/- percentage
					pct := float64(config.Jitter) / 100.0
					delta := float64(d) * pct
					// Random range [-delta, +delta]
					r := rand.Float64()*2*delta - delta
					d = time.Duration(float64(d) + r)
					if d < 0 {
						d = 0
					}
				}
			...
			if err == nil && resp.StatusCode == 429 && retries < maxRetries {
				...
				// Nnetote: resp.Headers is []http.Header
				for _, h := range resp.Headers {
					if nethttp.CanonicalHeaderKey(h.Key) == "Retry-After" {
						// Try parsing as integer seconds
						if s, err := strconv.Atoi(h.Value); err == nil {
						...
						// Reset exponential backoff if explicit header found? 
						// Usually yes, but let's keep it simple.
						break
					}
				}
				...
				log.Debug().
					Str("target", j.t.String()).
					Float64("waitTime", waitTime.Seconds()).
					Msg("429 Too Many Requests - Backing off")

				time.Sleep(waitTime)

				// Release the 429 response before retrying
				// Iterate and release chain
				for r := resp.Next; r != nil; {
					next := r.Next
					http.ReleaseResponse(r)
					r = next
				}
				// Release head resources
				resp.Reset()
				
				// Exponential Backoff if no Retry-After or just increment
				if waitTime == backoff {
					backoff *= 2
			...
			break // Break loop if success or error or max retries
		}
		...
		if err != nil {
			...
			// Phase 7: Replay Proxy Integration
			if config.ReplayProxy != "" {
				go ReplayRequest(j.t, route, config.ReplayProxy)
			}
