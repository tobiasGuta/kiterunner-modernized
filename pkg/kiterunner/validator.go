...
func (v *WildcardResponseValidator) Validate(r http.Response, wildcardResponses []WildcardResponse, c *Config) error {
	...
	for _, wr := range wildcardResponses {
		...
		if r.StatusCode == wr.DefaultStatusCode ||
			...
			// Phase 3: Auto-Calibration Soft 404 Detection
			if c.Calibrate {
				// Check for small variance (Soft 404 signature)
				// Variance allowed: 5 bytes
				diff := r.BodyLength - wr.DefaultContentLength
				if diff < 0 {
					diff = -diff
				}
				if diff <= 5 {
					log.Trace().Int("len", r.BodyLength).
						Int("baselineLines", wr.DefaultContentLength).
						Int("diff", diff).
						Msg("failed on soft 404 length match (calibration)")
