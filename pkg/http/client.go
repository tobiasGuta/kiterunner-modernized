...
type Config struct {
	...
	// WriteAbsoluteURI forces the client to write the absolute URI in the request line.
	// This is required for non-TLS proxying
	WriteAbsoluteURI bool
...
func DoClient(c *HTTPClient, req Request, config *Config) (Response, error) {
	...
	if config.WriteAbsoluteURI && !c.IsTLS {
		// Construct absolute URI (http://host/path)
		// Host is passed in header or client addr? HostClient uses Addr.
		// Use c.Addr (host) and freq.URI().PathOriginal()
		// freq.URI().PathOriginal() includes query string?
		// URI().RequestURI() includes path and query string
		// scheme is http because IsTLS is false
		fullURI := "http://" + c.Addr + string(freq.URI().RequestURI())
		freq.SetRequestURI(fullURI)
	}
